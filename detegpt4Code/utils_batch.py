...
def _make_causal_mask_2(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0, device: torch.device = None):
    """
    Make causal mask used for bi-directional self-attention.
    """
    bsz, tgt_len = input_ids_shape
    # mask = torch.full((tgt_len, tgt_len), torch.tensor(torch.finfo(dtype).min / 10))
    mask = torch.full((tgt_len, tgt_len), torch.tensor(-1e4), device=device)
...
transformers.models.xglm.modeling_xglm._make_causal_mask = _make_causal_mask_2
...
class InfillingModel:
    ...
    def batched_generate(self, inputs: List[str], max_to_generate: int=128, temperature: float=0.2, trim: bool=True, stop_words=None):
        ...
        with torch.no_grad():
            outputs = self.model.generate(input_ids=batch.input_ids, attention_mask=batch.attention_mask, do_sample=True, \
                                          top_p=0.95, temperature=temperature, max_length=max_length, stopping_criteria=stopping_criteria)
